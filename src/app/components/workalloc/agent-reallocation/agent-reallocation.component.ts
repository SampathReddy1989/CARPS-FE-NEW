...
@Component({
  selector: "app-agent-reallocation",
  templateUrl: "./agent-reallocation.component.html",
  styleUrls: ["./agent-reallocation.component.scss"],
})
export class AgentReallocationComponent implements OnInit {
  @Input() name: string;
  @Input() refreshDataSubject: Subject<boolean> = new Subject<boolean>();
  ...
  constructor(
    private http: AgentReallocationService,
    private dialog: MatDialog,
    private dialogService: DialogService
  ) {}

  ngOnInit() {
    this.refreshDataSubject.subscribe((response) => {
      if (response) {
  ...
  private loadCache() {
    ...
    // let agentAssignData: AgentAllocationOperations = UiCacheStore.fromCache(
    //   this.name.trim()
    // );

    // if (agentAssignData) {
    //   delete agentAssignData.agentlist;
    //   delete agentAssignData.accountCount;
    //   this.http.postAccountAllocationUser(agentAssignData).subscribe((data) => {
    //     if (data) {
  ...
  //   delete data.impid;
  // }
  // this.http.postAccountAllocationUser(data).subscribe((data) => {
  //   if (data) {
  //     this.tabledata.emit(data);
  //   }
  // });
  ...
  private evalAgent(agent: AgentAllocationOperations) {
    ...
    this.http.postAccountAllocationUser(agent).subscribe((data) => {
      if (data) {
        this.tabledata.emit(data);
      }
    });
